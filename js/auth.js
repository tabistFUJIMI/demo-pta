...
(function () {
  ...
  function showGate() {
    ...
    overlay.innerHTML = `
      <div style="position:fixed;inset:0;background:rgba(74,144,164,0.95);display:flex;align-items:center;justify-content:center;z-index:9999;backdrop-filter:blur(8px);">
        <div style="background:#fff;border-radius:16px;padding:40px;max-width:380px;width:90%;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,0.3);">
          ...
          <h2 style="font-size:20px;color:#4A90A4;margin-bottom:4px;">管理画面</h2>
          ...
          <input type="password" id="auth-password" placeholder="パスワード"
            ...
            onfocus="this.style.borderColor='#4A90A4'" onblur="this.style.borderColor='#e8ecf0'">
          <p id="auth-error" style="color:#e74c3c;font-size:13px;margin-top:8px;display:none;">パスワードが違います</p>
          <button id="auth-submit"
            style="width:100%;margin-top:16px;padding:14px;background:#4A90A4;color:#fff;border:none;border-radius:8px;font-size:15px;font-weight:700;cursor:pointer;transition:background 0.2s;"
            onmouseover="this.style.background='#5BA0B4'" onmouseout="this.style.background='#4A90A4'">ログイン</button>
        </div>
      </div>
    `;
